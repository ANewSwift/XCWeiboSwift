...
class WBStatusViewModel: CustomStringConvertible {
    ...
    /// 配图视图大小
    var pictureViewSize = CGSize.init()
    ...
    func updateRowHeight() {
        ...
        let viewSize = CGSize.init(width: UIScreen.cz_screenWidth() - 2 * margin, height: CGFloat.init(MAXFLOAT))
    ...
    func updateSingleImageSize(image: UIImage) {
        ...
        if size.width < minWidth {
            ...
            size.width = size.width * image.size.height / image.size.width / 4
        }
    ...
    private func calcPictureViewSize(count: Int?) -> CGSize{
        ...
        let height = WBStatusPictureViewOutterMargin + CGFloat.init(row) * WBStatusPictureItemWidth + CGFloat.init(row - 1) * WBStatusPictureViewInnerMargin
        
        return CGSize.init(width: WBStatusPictureViewWidth, height: height)
        
    }
    
    /// 给定一个数字，返回对应的描述结果(计算转发，评论，点赞的数字，合理显示)
    private func countString(count: Int,defaultStr: String) -> String {
        ...
        // 超过一万,保留两位小数
        return String.init(format: "%.02f 万", Double.init(count) / 10000)
    }
}

...
class WBWelcomeView: UIView {
    ...
    override func awakeFromNib() {
        ...
        // 1、取头像地址
        guard let urlString = WBNetWorkManager.shared.userAccount.avatar_large,let url = URL.init(string: urlString) else {
            return
        }
        
        // 2、设置头像 - 如果网络图像没有下载完成，先显示占位图像
        // 如果不指定占位图像，之前设置的图像会被清空！
        iconView.setImageWith(url, placeholderImage: UIImage.init(named: "avatar_default_big"))
        
        iconView.setImageWith(url)
